...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (!name?.trim() || !email?.trim() || !subject?.trim() || !message?.trim()) {
      return errRes("Missing required fields", 400);
    }
...
<div style="max-width:600px;margin:0 auto;background:#13131a;border-radius:12px;border:1px solid #1e1e2e;overflow:hidden;">
  <div style="background:linear-gradient(135deg,#00d4ff,#b24bf3);padding:24px;text-align:center;">
    ...
    <p style="margin:6px 0 0;color:rgba(255,255,255,0.8);font-size:13px;">AI Recaps Maker</p>
  </div>
  <div style="padding:24px;">
    <div style="background:#1a1a2e;border-left:4px solid #00d4ff;border-radius:8px;padding:16px;margin-bottom:20px;">
      <table style="width:100%;border-collapse:collapse;">
        <tr><td style="padding:5px 0;color:#888;font-size:12px;width:70px;font-weight:700;">NAME</td><td style="color:#fff;font-size:14px;font-weight:700;">${safe(name.trim())}</td></tr>
        <tr><td style="padding:5px 0;color:#888;font-size:12px;font-weight:700;">EMAIL</td><td><a href="mailto:${safe(email.trim())}" style="color:#00d4ff;font-size:14px;">${safe(email.trim())}</a></td></tr>
        <tr><td style="padding:5px 0;color:#888;font-size:12px;font-weight:700;">SUBJECT</td><td style="color:#fff;font-size:14px;">${safe(subject.trim())}</td></tr>
      </table>
    </div>
    <h3 style="margin:0 0 10px;color:#b24bf3;font-size:12px;font-weight:700;letter-spacing:1px;">MESSAGE</h3>
    ...
    <div style="margin-top:20px;text-align:center;">
      <a href="mailto:${safe(email.trim())}?subject=Re: ${encodeURIComponent(subject.trim())}" style="display:inline-block;background:linear-gradient(135deg,#00d4ff,#b24bf3);color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:700;font-size:13px;">↩ Reply to ${safe(name.trim())}</a>
    </div>
  </div>
  <div style="padding:12px 24px;border-top:1px solid #1e1e2e;text-align:center;">
    ...
    } catch (e) { console.warn("DB save failed (non-fatal):", e); }
