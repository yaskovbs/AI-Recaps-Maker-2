...
export default function SignUpScreen() {
  ...
  const { signUp, signInWithGoogle, setGeminiApiKey: saveGeminiApiKey } = useAuth();
  ...
  const handleSignUp = async () => {
    ...
    if (!fullName || !email || !password || !confirmPassword) {
      setError(isRTL ? 'אנא מלא את כל השדות' : 'Please fill all fields');
      return;
    }
    ...
    if (password.length < 6) {
      setError(isRTL ? 'הסיסמה חייבת להיות לפחות 6 תווים' : 'Password must be at least 6 characters');
      return;
    }
    ...
    try {
      ...
      if (geminiApiKey && geminiApiKey.trim()) {
        ...
        if (!saved) {
          ...
          console.warn('⚠️ Gemini API key not saved during signup');
        } else {
          setKeySaved(true);
          console.log('✅ Gemini API key saved during signup');
        }
      }
      ...
      setError(error.message || (isRTL ? 'ההרשמה נכשלה' : 'Sign up failed'));
          ...
          >
            ...
            <View style={styles.header}>
              ...
              <Text style={styles.welcomeSubtitle}>
                {isRTL ? 'הצטרף אלינו והתחל ליצור סיכומים מדהימים' : 'Join us and start creating amazing recaps'}
              </Text>
            </View>
            ...
            <View style={styles.formContainer}>
              ...
              <View style={styles.inputContainer}>
                ...
                <TextInput
                  style={styles.input}
                  placeholder={isRTL ? 'סיסמה (לפחות 6 תווים)' : 'Password (min 6 characters)'}
                  placeholderTextColor={Colors.textMuted}
                  value={password}
              ...
              >
                ...
                <Text style={styles.googleButtonText}>
                  {isRTL ? 'הירשם עם Google' : 'Sign Up with Google'}
                </Text>
              </Pressable>
